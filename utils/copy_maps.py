...
def copy_custom_maps(src, dst):
    assert src.exists(), "Source dir for custom maps maps folder not found at {}".format(src)
    assert dst.exists(), "Destination dir for custom maps maps folder not found at {}".format(dst)
    print("\nCopying custom maps to {}".format(dst))
    os.system('cp -arv {}/*.yaml {}/'.format(src, dst))


custom_maps_path = Path(__file__).resolve().parent.parent.joinpath('maps')
